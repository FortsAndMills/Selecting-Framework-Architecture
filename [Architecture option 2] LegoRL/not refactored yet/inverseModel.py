...
class InverseModelHead(Head):
    def state_repr(self, state):
        return self.feature_extractor_net(state)
...
class InverseModelNetwork(InverseModelHead):
    ...
    def forward(self, state_repr, next_state_repr):
        return self.head(torch.cat([state_repr, next_state_repr], dim=1))
...
def InverseModel(parclass):
  ...
  class InverseModel(parclass):
    ...
    PARAMS = parclass.PARAMS | Head.PARAMS("InverseModel") | {"InverseModelHead", 
                                        "curiosity_batch_size", "curiosity_coeff",
    ...
    def __init__(self, config):
        ...
        self.inverse_model = self.config.InverseModelHead(self.config, "InverseModel", personal_loss_plot=True).to(device)
    ...
    def intrinsic_motivation(self, state, action, next_state, done):
        ...
        with torch.no_grad():
            ...
            return (1 - done) * (self.config.curiosity_coeff * self.action_prediction_loss(action, predicted_action).cpu().numpy())
    ...
    def optimize_curiosity(self):
        ...
        state_repr = self.inverse_model.state_repr(Tensor(state_b))
        next_state_repr = self.inverse_model.state_repr(Tensor(next_state_b))
        
        loss_b = self.action_prediction_loss(action_b, self.inverse_model(state_repr, next_state_repr))
